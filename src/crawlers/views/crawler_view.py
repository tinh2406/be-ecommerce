...
class CrawlerViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        crawler_id = CrawlerService.create(data.validated_data)
        return Response({"data": crawler_id})

    def retrieve(self, request, *args, **kwargs):
        pk = kwargs.get("pk")
        crawler = CrawlerService.get(pk)

        return Response(DetailPeriodicTaskSerializer(crawler).data)

    def update(self, request, *args, **kwargs):
        pk = kwargs.get("pk")
        instance = CrawlerService.get(pk)

        data = CrawlerSerializer(instance, data=request.data)
        data.is_valid(raise_exception=True)

        crawler_id = CrawlerService.update(instance, data.validated_data)
        return Response({"data": crawler_id})
